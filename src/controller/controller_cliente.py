...
class Controller_Card:
    def __init__(self):
        pass
        
    def inserir_cliente(self) -> Card:
        ...
        if self.verifica_existencia_cliente(oracle, cpf):
            ...
            novo_cliente = Card(df_cliente.cpf.values[0], df_cliente.nome.values[0])
    ...
    def atualizar_cliente(self) -> Card:
        # Cria uma nova conexão com o banco que permite alteração
        oracle = OracleQueries(can_write=True)
        oracle.connect()
        ...
        if not self.verifica_existencia_cliente(oracle, cpf):
            ...
            df_cliente = oracle.sqlToDataFrame(f"select cpf, nome from clientes where cpf = {cpf}")
            # Cria um novo objeto cliente
            cliente_atualizado = Card(df_cliente.cpf.values[0], df_cliente.nome.values[0])
            # Exibe os atributos do novo cliente
    ...
    def excluir_cliente(self):
        ...
        if not self.verifica_existencia_cliente(oracle, cpf):            
            ...
            cliente_excluido = Card(df_cliente.cpf.values[0], df_cliente.nome.values[0])
